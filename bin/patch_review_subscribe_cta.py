#!/usr/bin/env python3
"""Activa suscripción por correo en mirrors locales reviews/*.html (sin re-descargar)."""
...
from mirror_first_review import patch_review_html_subscribe  # noqa: E402


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Inserta meta review-subscribe-cta e icono de correo en reseñas locales."
    )
    parser.add_argument(
        "--reviews-dir",
    ...
    for path in sorted(reviews_dir.glob("*.html")):
        ...
        patched = patch_review_html_subscribe(original)
        if patched == original:
            continue
        changed += 1
        if args.dry_run:
